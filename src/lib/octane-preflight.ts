...
export async function ensureLocalOctaneReady(): Promise<OctaneReadinessResult> {
  ...
  const composerCheck = await checkComposerForOctane(path.join(projectRoot, 'composer.json'));
  const hasOctaneConfig = await fileExists(path.join(projectRoot, 'config', 'octane.php'));
...
async function checkComposerForOctane(
  ...
  const requireMap = (parsed as { require?: Record<string, unknown> }).require;
