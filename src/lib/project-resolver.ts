...
export async function resolveProjectContext(
    ...
    if (link !== null) {
        ...
        if (linkedApp.app !== null) {
            const mismatchError = linkedAppWorkspaceMismatchError(
                resolvedCredentials,
                linkedApp.app,
            );
...
export async function readStackFromConfigFile(
...
async function getAppById(input: {
    ...
    try {
        const endpoint = new URL(
            buildApiV1Endpoint(input.apiUrl, `/apps/${input.appId}`),
        );
        const response = await fetch(endpoint, {
            headers: buildApiHeaders({
                token: input.credentials.token,
...
function linkedAppWorkspaceMismatchError(
    ...
    if (
        ...
        app.workspace_id === credentials.selectedWorkspaceId
    ) {
        return null;
    }
