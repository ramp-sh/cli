...
export async function runCreateCommand(
    ...
    if (isInteractiveUi() && !options.quiet && !options.json) {
        const sourceLabel =
            sourceType === 'upload' ? 'Local upload' : 'GitHub repo';
    ...
    const selectedServer = await resolveServer(
        options.server,
        activeServers,
        servers.data,
    );
...
async function createUploadApp(
    ...
    if (name === '') {
        process.stderr.write(
            `${statusLine('error', 'App name is required.')}\n`,
        );
        return 1;
    }
    ...
    const response = await fetch(
        buildApiV1Endpoint(apiUrl, `/servers/${server.id}/apps/upload`),
        {
            method: 'POST',
    ...
    if (!response.ok || payload.ok !== true || !payload.app) {
        const errorMessages = extractErrors(
            payload.errors,
            payload.message,
            response.status,
        );
        process.stderr.write(
            `${statusLine('error', 'Failed to create app:')}\n`,
        );
    ...
    if (!options.quiet) {
        process.stdout.write(
            `${box([
                ...
                statusLine(
                    'info',
                    `Linked current directory: ${paint(linkPath, 'bold')}`,
                ),
                payload.deploy_queued === true
                    ? keyHint(
                          'Initial deploy queued. Deploy again anytime with `ramp upload`',
                      )
...
> {
    ...
    const startCommand = document.getIn([
        'services',
        serviceName,
        'start',
    ]) as unknown;
    ...
    if (typeof startCommand === 'string') {
        const nextStartCommand = rewriteCommandPort(
            startCommand,
            currentPort,
            suggestedPort,
        );
...
async function createRepoApp(
    ...
    const nameInput =
        options.name ?? (await askOrCancel(rl, 'App name (stack): '));
    const repositoryInput =
        options.repository ??
        (await askOrCancel(rl, 'Repository (owner/repo): '));
    const branchInput =
        options.branch ?? (await askOrCancel(rl, 'Branch [main]: '));
    ...
    if (name === '' || repository === '') {
        process.stderr.write(
            `${statusLine('error', 'App name and repository are required.')}\n`,
        );
        return 1;
    }

    const response = await fetch(
        buildApiV1Endpoint(apiUrl, `/servers/${server.id}/apps`),
        {
            method: 'POST',
            headers: buildApiHeaders({
                token: credentials.token,
...
async function resolveServer(
    ...
    if (typeof input === 'string' && input.trim() !== '') {
        ...
        const matchServer = (server: ServerItem) =>
            server.id === needle || server.name === needle;
