...
export async function runWorkspaceCommand(
    ...
    if (workspacesResult.error !== null) {
        process.stderr.write(
            `${statusLine('error', workspacesResult.error)}\n`,
        );
        return 1;
    }
    ...
    const currentWorkspace =
        workspaces.find((workspace) => workspace.is_current === true) ?? null;

    if (options.workspace && options.workspace.trim() !== '') {
        const targetWorkspace = findWorkspace(
            workspaces,
        ...
        if (targetWorkspace === null) {
            process.stderr.write(
                `${statusLine(
                    'error',
                    `Workspace '${options.workspace.trim()}' not found.`,
                )}\n`,
            );
            return 1;
        }
    ...
    if (workspaces.length === 0) {
        if (!options.quiet) {
            process.stdout.write(
                `${statusLine('info', 'No workspaces available.')}\n`,
            );
        }
            ...
            if (!options.quiet) {
                process.stdout.write(
                    `${box([
                        statusLine(
                            'success',
                            `Current workspace: ${paint(onlyWorkspace.name, 'bold')}`,
                        ),
    ...
    if (!process.stdin.isTTY || !isInteractiveUi()) {
        if (!options.quiet) {
            ...
            process.stdout.write(
                `${keyHint('Use `ramp workspace <workspace>` to switch.')}\n`,
            );
        }
...
async function switchWorkspace(input: {
    ...
    if (input.workspace.is_current === true) {
        if (input.json) {
            process.stdout.write(
                `${JSON.stringify({ data: input.workspace }, null, 2)}\n`,
            );
        } else if (!input.quiet) {
            process.stdout.write(
                `${box([
                    statusLine(
                        'success',
                        `Current workspace: ${paint(input.workspace.name, 'bold')}`,
                    ),
    ...
    try {
        const response = await fetch(
            buildApiV1Endpoint(
                input.apiUrl,
                `/workspaces/${input.workspace.id}/switch`,
            ),
            {
                method: 'POST',
                headers: buildApiHeaders({
        ...
        if (!response.ok) {
            process.stderr.write(
                `${statusLine(
                    'error',
                    await describeApiError(response, 'Failed to switch workspace'),
                )}\n`,
            );
            return 1;
        }
        ...
        process.stderr.write(
            `${statusLine('error', `Failed to switch workspace: ${message}`)}\n`,
        );
        return 1;
    }
    ...
    if (!input.quiet) {
        ...
        process.stdout.write(
            `${box([
                statusLine(
                    'success',
                    `Current workspace: ${paint(switchedWorkspace.name, 'bold')}`,
                ),
...
function findWorkspace(
    ...
    return (
        ...
        workspaces.find(
            (workspace) => workspace.name.trim().toLowerCase() === normalized,
        ) ??
        null
    );
}
...
function formatWorkspaceLine(workspace: WorkspaceItem): string {
    const current =
        workspace.is_current === true ? badge('current', 'success') : '';
    ...
    return `${paint(workspace.name, 'bold')} ${current}`.trimEnd()
