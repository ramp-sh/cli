...
export async function runImportRenderCommand(options: ImportRenderCommandOptions): Promise<number> {
  ...
  try {
    ...
    process.stderr.write(`${statusLine('error', `Import failed: ${message}`)}\n`);
    return 1;
  }
}

async function readBlueprintInput(options: ImportRenderCommandOptions): Promise<string | null> {
  if (options.stdin) {
    let input = '';
