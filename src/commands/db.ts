...
export async function runDbBackupCommand(options: BaseOptions & { list?: boolean }): Promise<number> {
    ...
    if (options.list) {
        ...
        if (!listResponse.ok) {
            process.stderr.write(
                `${await describeApiError(listResponse, 'Failed to list backups')}\n`,
            );
            return 1;
        }
...
export async function runDbRestoreCommand(
    ...
    const response = await fetch(
        buildApiV1Endpoint(resolved.context.apiUrl, `/apps/${resolved.context.app.id}/db/backups/${backupId}/restore`),
        {
            method: 'POST',
            headers: buildApiHeaders({
                token: resolved.context.token,
