...
export async function runUnlinkCommand(options: UnlinkCommandOptions): Promise<number> {
  ...
  if (linkPath === null) {
    if (options.json) {
      ...
      process.stdout.write('No .ramp/config.json found for this directory.\n');
    }
  ...
  if (options.json) {
    process.stdout.write(`${JSON.stringify({ removed: true, path: linkPath }, null, 2)}\n`);
  } else if (!options.quiet) {
