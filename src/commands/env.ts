...
export async function runEnvList(options: BaseEnvOptions): Promise<number> {
    ...
    const endpoint = new URL(
        buildApiV1Endpoint(context.apiUrl, `/apps/${context.appId}/env`),
    );
    ...
    if (!response.ok) {
        process.stderr.write(
            `${await describeApiError(response, 'Failed to list env vars')}\n`,
        );
        return 1;
    }
...
export async function runEnvDelete(
    options: BaseEnvOptions & { key: string },
): Promise<number> {
    const context = await resolve(options);
...
export async function runEnvPull(
    options: BaseEnvOptions & { output: string },
): Promise<number> {
    const context = await resolve(options);
    ...
    const endpoint = new URL(
        buildApiV1Endpoint(context.apiUrl, `/apps/${context.appId}/env/export`),
    );
    ...
    if (!response.ok) {
        process.stderr.write(
            `${await describeApiError(response, 'Failed to pull env vars')}\n`,
        );
        return 1;
    }
...
export async function runEnvPush(
    options: BaseEnvOptions & { file: string },
): Promise<number> {
    const context = await resolve(options);
    ...
    if (!options.quiet) {
        process.stdout.write(
            `Imported ${payload.imported ?? 0} env vars from ${options.file}.\n`,
        );
    }
