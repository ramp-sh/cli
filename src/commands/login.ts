...
export async function runLoginCommand(options: LoginCommandOptions): Promise<number> {
  ...
  try {
    ...
    if (
      ...
      process.stderr.write(`${statusLine('error', 'Unexpected login response from API.')}\n`);
      return 1;
    }
    ...
    while (!interrupted) {
      ...
      if (
        pollPayload.status === 'approved' &&
        typeof pollPayload.token === 'string' &&
        pollPayload.token !== ''
      ) {
      ...
      if (pollPayload.status === 'invalid' || !pollResponse.ok) {
        ...
        process.stderr.write(`${statusLine('error', pollError)}\n`);
        return 1;
      }
    ...
    process.stderr.write(`${statusLine('error', `Login failed: ${message}`)}\n`);
...
async function loginWithToken(
  ...
  try {
    ...
    if (!response.ok) {
      process.stderr.write(`${statusLine('error', 'Invalid API token.')}\n`);
      return 1;
    }
