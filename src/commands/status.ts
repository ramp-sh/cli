...
import { badge, kvTable, paint, statusLine, toneForStatus } from '../lib/ui.js';
...
export async function runStatusCommand(options: StatusCommandOptions): Promise<number> {
  ...
  if (!app) {
    process.stderr.write(`${statusLine('error', 'Invalid status response from API.')}\n`);
    return 1;
  }
