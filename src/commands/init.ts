...
export async function runInitCommand(options: InitOptions): Promise<number> {
    ...
    if (!isInteractive && !options.yes) {
        return outputError(
            'Non-interactive init requires `--yes` to use defaults.',
            options,
        );
    }
    ...
    try {
        ...
        const selectedTemplate =
            template ?? (options.yes ? 'custom' : await chooseTemplate());
        ...
        const config = await buildTemplateConfig(
            selectedTemplate,
            stack,
            options,
            rl,
        ...
        if (!options.print && exists) {
            if (options.force) {
                ...
                const fileMode = options.yes
                    ? 'merge'
                    : await chooseExistingFileMode();
                ...
                if (fileMode === 'overwrite') {
                    ...
                    const merged = await mergeWithExistingConfig(
                        targetPath,
                        config,
                    );
...
async function chooseTemplate(): Promise<Template | null> {
    return selectWithArrows('Select template', [
        ...
        },
        {
            ...
            description:
                'Laravel web + queue worker + FrankenPHP Octane defaults',
...
async function buildCustomTemplate(
    ...
    const normalizedServices =
        services.length > 0 ? services : (['web'] as ServiceKind[]);
    ...
    const includeExampleRefs = !options.yes
        ? await askOrCancel(
              rl,
              'Add sample Ramp env mappings to the web service? [y/N]: ',
          )
        : 'n';
...
async function buildLaravelTemplate(
    ...
    const domains = await askForOptionalDomains(
        rl,
        options,
        'Custom domain (optional): ',
    );
...
async function askForOptionalDomains(
    ...
    while (true) {
        ...
        process.stderr.write(
            'Enter a bare domain like api.example.com, without http:// or paths.\n',
        );
    }
}
...
function outputInitResult(result: InitResult, options: InitOptions): number {
    ...
    if (!options.quiet && result.path !== null) {
        ...
        if (result.conflicts.length > 0) {
            process.stdout.write(
                `${box([
                    statusLine(
                        'info',
                        `Preserved existing entries: ${result.conflicts.join(', ')}`,
                    ),
                ])}\n`,
            );
        }
        ...
        process.stdout.write(
            `${keyHint('Next: ramp validate, then ramp deploy')}\n`,
        );
    }
