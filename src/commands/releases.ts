...
import { badge, paint, sectionHeader, statusLine } from '../lib/ui.js';
...
export async function runReleasesCommand(options: ReleasesCommandOptions): Promise<number> {
  ...
  try {
    ...
    process.stderr.write(`${statusLine('error', `Failed to fetch releases: ${message}`)}\n`);
    return 1;
  }
}
