...
export async function runLinkCommand(options: LinkCommandOptions): Promise<number> {
  ...
  const stack =
    options.app ?? (configFilePath ? await readStackFromConfigFile(configFilePath) : null);
  ...
  if (matches.apps.length === 0) {
    if (stack !== null) {
      ...
      if (fallback.error !== null) {
        process.stderr.write(`${statusLine('error', fallback.error)}\n`);
        return 1;
      }
  ...
  if (selected === null) {
    process.stderr.write(`${statusLine('error', 'No app selected.')}\n`);
    return 1;
  }
