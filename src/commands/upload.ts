...
export async function runUploadCommand(options: UploadCommandOptions): Promise<number> {
  ...
  if (!options.quiet && !options.json) {
    process.stdout.write(`${statusLine('info', `Packaging ${paint(app.stack, 'bold')}...`)}\n`);
  }
  ...
  if (!options.quiet && !options.json) {
    process.stdout.write(`${statusLine('info', `Archive: ${archiveSize}`)}\n`);
  ...
  try {
    ...
    process.stderr.write(`${statusLine('error', `Upload failed: ${message}`)}\n`);
    return 1;
  }
}
