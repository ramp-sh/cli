...
export async function runAiBridgeCommand(
    ...
    const response = await fetch(
        buildApiV1Endpoint(
            ...
            `/apps/${resolved.context.app.id}?context=cli`,
        ),
        {
    ...
    if (preflight.status !== 0) {
        ...
        process.stderr.write(
            `${statusLine('error', 'SSH connection failed.')}\n`,
        );
        if (stderr) {
    ...
    const whichCheck = spawnSync(
        'ssh',
        [...sshBase, `bash -lic 'command -v ${binary}'`],
