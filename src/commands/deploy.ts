...
export async function runDeployCommand(
    ...
    if (appMeta.deploy_mode === 'upload') {
        process.stderr.write(
            'This app uses upload deploys. Run `ramp upload` instead.\n',
        );
        return 1;
    }
    ...
    const response = await fetch(
        buildApiV1Endpoint(
            ...
            `/apps/${resolved.context.app.id}/deploy`,
        ),
        {
            method: 'POST',
    ...
    if (!response.ok) {
        process.stderr.write(
            `${await describeApiError(response, 'Failed to trigger deploy')}\n`,
        );
        return 1;
    }
...
async function confirmGitStateBeforeDeploy(
    options: DeployCommandOptions,
): Promise<boolean> {
    ...
    if (!options.quiet) {
        process.stdout.write(
            'Your local git state differs from what remote deploy will pull.\n',
        );
        ...
        if (state.aheadCount > 0) {
            process.stdout.write(
                `- ${state.aheadCount} local commit(s) not pushed.\n`,
            );
        }

        process.stdout.write(
            'Deploy pulls from GitHub, not your local filesystem.\n',
        );
        process.stdout.write(
            'Tip: commit/push first if you want local changes included.\n',
        );
    }
    ...
    try {
        const answer = (
            await askOrCancel(rl, 'Continue deploy anyway? [y/N]: ')
        )
            ?.trim()
            .toLowerCase();
